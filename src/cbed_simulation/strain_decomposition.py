...
class StrainResult(NamedTuple):
    ...
    def __getitem__(self, sl: slice | int | Sequence[int | slice]) -> StrainResult:
        if np.isscalar(self.e_xx):
            raise ValueError("Cannot slice into scalar strain result")
        return StrainResult(
            e_xx=self.e_xx[sl],
            e_xy=self.e_xy[sl],
            e_yy=self.e_yy[sl],
    ...
    def __format__(self, format_str: str) -> str:
        return (
            ...
            f"e_xx={format(self.e_xx, format_str)}, "
            f"e_xy={format(self.e_xy, format_str)}, "
            f"e_yy={format(self.e_yy, format_str)}, "
            f"theta={format(self.theta, format_str)}, "
            f"rotation={format(self.rotation, format_str)}"
            ")"
        )

    @staticmethod
    def _rotate_fn():
        return _rotate_strain
    ...
    def rotate_rad(self, rotation: float, absolute: bool = False) -> StrainResult:
        if absolute:
            ...
            _e_xx, _e_xy, _e_yy = self._rotate_fn()(
                self.e_xx,
                self.e_xy,
                self.e_yy,
                rotation,
...
def _rotate_vec(e_xx, e_xy, e_yy, rotation_rad):
    if np.allclose(rotation_rad, 0.):
        return e_xx, e_xy, e_yy

    costheta = np.cos(-1 * rotation_rad)
    sintheta = np.sin(-1 * rotation_rad)
    # [
    #     [np.cos(angle), -np.sin(angle)],
    #     [np.sin(angle),  np.cos(angle)],
    # ]

    _e_xx = e_xx * costheta - e_yy * sintheta
    _e_yy = e_xx * sintheta + e_yy * costheta

    return _e_xx, e_xy, _e_yy
...
def compute_strain_large_def(
    ...
    exx, exy, eyy, rot = _to_strain(d11, d12, d21, d22)
...
def _to_strain(d11, d12, d21, d22):
...
class VectorResult(StrainResult):
    @staticmethod
    def _rotate_fn():
        return _rotate_vec
